...
export function buildPanelNavigationMarkup(activeType = '') {
    const buttons = PANEL_NAV_ITEMS.map((item) => {
        ...
        return '<button class="panel-modal-nav-btn ' + (active ? 'active' : '') + '" type="button" data-panel-nav="' + escapeHtml(item.id) + '" aria-label="' + escapeHtml(label) + '" title="' + escapeHtml(title) + '" data-tooltip="' + escapeHtml(label) + '" aria-current="' + (active ? 'page' : 'false') + '"><i class="fas ' + normalizeIconClass(item.icon) + '"></i><span>' + escapeHtml(item.label) + '</span></button>';
    }).join('');
